...
exports.form = function (req,res){
   ...
   /* req.checkBody('userName','Must insert a user!').notEmpty();
    req.checkBody('userName','Incorrect user name format!Use just alphanumeric characters!').isAlphanumeric();
    ...
    req.checkBody('userName','Incorrect password format!Use just alphanumeric characters!').isAlphanumeric();

    req.checkBody('firstName','Must insert your fisrt name!').notEmpty();
    req.checkBody('firstName','Incorrect first name format !').isAlpha();

    req.checkBody('lastName','Must insert your last name!').notEmpty();
    req.checkBody('firstName','Incorrect last name format!').isAlpha();

    req.checkBody('CNP','Must insert a your CNP!').notEmpty();
    req.checkBody('CNP','Incorrect CNP format!').isNumeric();
    req.checkBody('CNP','Password length minim 8!').isLength({min:3});*/

    var errors = req.validationErrors();
    console.log(errors);
    if (errors) {
        res.render('./createAccount.ejs', {error: errors});
    }
    else {
        var user  = {
